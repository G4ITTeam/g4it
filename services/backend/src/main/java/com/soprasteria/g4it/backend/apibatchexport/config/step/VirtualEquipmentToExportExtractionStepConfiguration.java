 ...
 */
@Configuration
public class VirtualEquipmentToExportExtractionStepConfiguration {

    @Bean
    public Flow virtualEquipmentToExportExtractionFlow(final Step virtualEquipmentToExportExtractionStep) {
        return new FlowBuilder<SimpleFlow>("virtualEquipmentToExportExtractionFlow")
                .start(virtualEquipmentToExportExtractionStep)
                .build();
    }
     ...
     * @param extractVirtualEquipmentToExportDataReader RepositoryItemReader to extract VirtualEquipment data from database.
     * @param extractedVirtualEquipmentToExportWriter   FlatFileItemReader to write extraction file.
     * @param chunkValue                                chunk size.
     * @return the configured Step.
     */
    @Bean
    public Step virtualEquipmentToExportExtractionStep(final JobRepository jobRepository,
                                                       final PlatformTransactionManager transactionManager,
                                                       final RepositoryItemReader<VirtualEquipment> extractVirtualEquipmentToExportDataReader,
                                                       final FlatFileItemWriter<VirtualEquipment> extractedVirtualEquipmentToExportWriter,
                                                       @Value("${export.batch.chunk}") final Integer chunkValue) {
        return new StepBuilder("virtualEquipmentToExportExtractionStep", jobRepository)
                .<VirtualEquipment, VirtualEquipment>chunk(chunkValue, transactionManager)
                .reader(extractVirtualEquipmentToExportDataReader)
                .writer(extractedVirtualEquipmentToExportWriter)
                .taskExecutor(virtualEquipmentToExportTaskExecutor())
                .build();
    }
     ...
     */
    @Bean
    public TaskExecutor virtualEquipmentToExportTaskExecutor() {
        return new SimpleAsyncTaskExecutor("virtualEquipmentToExportTaskExecutor");
    }
    ...
    @Bean
    @StepScope
    public RepositoryItemReader<VirtualEquipment> extractVirtualEquipmentToExportDataReader(final VirtualEquipmentRepository virtualEquipmentRepository,
        ...
        return builder.name("extractVirtualEquipmentToExportDataReader")
    ...
    @Bean
    @StepScope
    public FlatFileItemWriter<VirtualEquipment> extractedVirtualEquipmentToExportWriter(
        ...
        return new FlatFileItemWriterBuilder<VirtualEquipment>()
                .name("extractedVirtualEquipmentToExportWriter")
