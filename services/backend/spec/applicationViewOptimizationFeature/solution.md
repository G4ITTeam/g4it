...
→ 200: ApplicationFiltersRest { environment[], equipmentType[], lifeCycle[], domain[], subDomain[] }
```
...
- Backend: a single native SQL query computes all five distinct lists in one round
  trip using `array_agg(DISTINCT column) FILTER (WHERE column IS NOT NULL)` per
  dimension, grouped implicitly over the whole `task_id` scope (no `GROUP BY` needed
  since the result is one row). `domain`/`subDomain` are read via the same
  `filters[1]`/`filters[2]` array-indexing used throughout §4.1-§4.3.
- No pagination needed: result size is bounded by the number of *distinct* values per
  dimension (typically tens, not the number of rows), so there is no heap/memory risk
  even though the query scans the whole task's rows to compute the `DISTINCT`s
  DB-side.
...
- **§4.6's distinct filter values are computed 100% DB-side in a single native query**
  (`array_agg(DISTINCT ...) FILTER (WHERE ... IS NOT NULL)` per dimension) — no
  in-memory `Set`/`distinct()` building in Java, and no N+1 `SELECT DISTINCT col`
  queries. Result size is bounded by distinct-value cardinality (typically tens of
  values), not row count, so there is no heap/memory risk even for large tasks.
